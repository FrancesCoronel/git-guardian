$(document).ready(function() {
    ...
    var getHeadline = function(date) {
        var prettyDate = moment(date).format("YYYYMMDD");
        var url = "https://api.nytimes.com/svc/search/v2/articlesearch.json";
        url += '?' + $.param({
            'api-key': "c7546d5b745543dba769f76235cedbfb",
            'begin_date': prettyDate,
            'end_date': prettyDate,
            'sort': "newest",
        ...
        $.ajax({
            ...
            var matches = [];
            $.each(articles.response.docs, function(key, article) {
                if (date.diff(article.pub_date, 'minutes') < 30) {
                    matches.push(article.headline.main);
                } else {
                    return "Could not find a relevant headline!";
                }
                return matches[0];
            });
